...
class TechnicianController extends Controller
{
    ...
    public function user_save(Request $request)
    {
        ...
        if($validator->fails())
        {
            return redirect()->to('/backend/technician/user-list')->with('error','All Fields are required!');
        }

        if(User::where(['email' => $request->email])->count() > 0)
        {
            return redirect()->to('/backend/technician/user-list')->with('error','Email already exists!');
        }
        DB::beginTransaction();
        $user = new User();
        $user->uuid = Str::uuid();
        ...
        DB::commit();
        return redirect()->to('/backend/technician/user-create')->with('message','Account create successfully!');
    ...
    public function user_list()
    {
        $records =  User::whereHas('roles', function($sub_query) {
            $sub_query->where('roles.name', '=', 'technician');
        })->get();
        return view('admin.technician.list', compact('records'));
    }
    ...
    public function user_edit($uuid)
    {
        ...
        if($user)
        {
            return view('admin.technician.edit', compact(['user']));
        }
        return redirect()->to('/backend/technician/user-list')->with('error', 'Record Not Found!');
    }
    ...
    public function user_update(Request $request)
    {
        ...
        if($validator->fails())
        {
            return redirect()->to('/backend/technician/user-edit/'  . $request->id)->with('error', 'All Fields are required!');
        }
        ...
        if($user)
        {
            ...
            return redirect()->to('/backend/technician/user-list')->with('message', 'Record Updated Successfully!');
        }

        return redirect()->to('/backend/technician/user-list')->with('error', 'Something went wrong!');
